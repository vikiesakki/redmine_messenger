...
class Messenger  
  ...
  def teams_message(issue, type, user)
    ...
    if type == 'created'
      "<div><div>\n<div><p><a href='#{Messenger.object_url(issue.project)}'>#{issue.project.name}</a> issue <a href='#{Messenger.object_url(issue)}'>#{issue.tracker.name} #{issue.id}: #{issue.subject}</a> created by #{user.name}</p><hr>#{markdown.render(issue.description.to_s.truncate(200)).html_safe}<p>#{render_email_issue_attributes(issue, issue.author, true)}</p>\n\n</div>\n\n\n</div>\n</div>"
    else
